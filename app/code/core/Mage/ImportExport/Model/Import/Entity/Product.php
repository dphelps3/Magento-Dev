...
class Mage_ImportExport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Abstract
{
    ...
    {
        return $this->_initSkusBySkuList();
    }

    /**
     * Initialize existent product SKUs
     * by specified SKU list
     *
     * @param null|array $skus
     * @return Mage_ImportExport_Model_Import_Entity_Product
     */
    protected function _initSkusBySkuList($skuList = null)
    {
        ...
        $select = $adapter->select()
            ->from($resource->getTable('catalog/product'), $columns);

        if (is_array($skuList)) {
            $bunchSize = Mage::helper('importexport')->getBunchSize();

            while (count($skusBunch = array_splice($skuList, 0, $bunchSize))) {
                $select->reset(Zend_Db_Select::WHERE)
                    ->where('sku IN (?)', $skusBunch);

                $this->_addOldSkuEntries($adapter->fetchAll($select));
            }
        } else {
            $this->_addOldSkuEntries($adapter->fetchAll($select));
        }
    ...
    /**
     * Initialize existent product SKUs
     * for specified bunch of rows
     *
     * @param array $bunch
     */
    protected function _initSkusForBunch(&$bunch)
    {
        $skuList = array();

        foreach ($bunch as $row) {
            if (isset($row[self::COL_SKU]) && !isset($this->_oldSku[$row[self::COL_SKU]])) {
                $skuList[] = $row[self::COL_SKU];
            }
            if (isset($row['_super_products_sku']) && !empty($row['_super_products_sku'])) {
                $skuList[] = $row['_super_products_sku'];
            }
            if (isset($row['_associated_sku']) && !empty($row['_associated_sku'])) {
                $skuList[] = $row['_associated_sku'];
            }
        }

        $this->_initSkusBySkuList($skuList);
    }

    /**
     * Add list of entries to _oldSku array
     *
     * @param array $entries
     */
    protected function _addOldSkuEntries($entries)
    {
        foreach ($entries as $entry) {
            ...
            $this->_oldSku[$sku] = array(
                'type_id'        => $typeId,
                'attr_set_id'    => $entry['attribute_set_id'],
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                if ($lastProductId != $productId) {
                    $lastStoreCode = Mage_Catalog_Model_Abstract::DEFAULT_STORE_ID;
                    $currentValueId = null;
                ...
                if ($typeSpecific[$type] === true && !empty($rowData['_custom_option_row_title'])) {
                    if (empty($rowData['_custom_option_store'])) {
                        ...
                        if ($lastStoreId != $storeId) {
                            ...
                            $currentValueId = key($customOptions[$typeTitleTable]);
                            $lastStoreId = $storeId;
                        } else {
                            $currentValueId++;
                        }
        ...
        foreach ($attributesData as $tableName => $skuData) {
            ...
            foreach ($skuData as $sku => $attributes) {
                ...
                foreach ($attributes as $attributeId => $storeValues) {
                    ...
                    if ($this->_getUrlKeyAttributeId() == $attributeId) {
                        /*
    ...
    /**
     * Init needed data for bunch
     *
     * @param array $bunch
     * @return Mage_ImportExport_Model_Import_Entity_Product
     */
    protected function _initDataForBunch(&$bunch)
    {
        $this->_initSkusForBunch($bunch);
        return $this;
    }
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                foreach ($this->_imagesArrayKeys as $imageCol) {
                    if (!empty($rowData[$imageCol])) { // 5. Media gallery phase
                        if (!array_key_exists($rowData[$imageCol], $uploadedGalleryFiles)) {
                            $result = $this->_uploadMediaFiles($rowData[$imageCol]);
                            if ($result) {
                                $mediaGallery[$rowSku][$result] = array(
                                    'attribute_id' => $this->_getMediaGalleryAttributeId(),
                                    'label'        => '',
                                    'position'     => 0,
                                    'disabled'     => 1,
                                    'value'        => $result
                            ...
                            $uploadedGalleryFiles[$rowData[$imageCol]] = $result;
                        }
                        $rowData[$imageCol] = $uploadedGalleryFiles[$rowData[$imageCol]];
                    }
                }
                if (!empty($rowData['_media_image'])) {
                    $mediaGallery[$rowSku][$rowData['_media_image']] = array(
                ...
                switch ($rowScope) {
                    case self::SCOPE_STORE:
                        $rowStore = isset($this->_storeCodeToId[$rowData[self::COL_STORE]])
                            ? $this->_storeCodeToId[$rowData[self::COL_STORE]]
                            : 0;
                        break;

                    case self::SCOPE_NULL:
                        break;
    ...
    /**
     * Get website store ids by store id
     *
     * @param int $storeId
     * @return array
     */
    protected function _getWebsiteStoreIds($storeId)
    {
        $storeIds = $this->_storeIdToWebsiteStoreIds[$storeId];

        if (!$storeIds) {
            $storeIds = array($storeId => $storeId);
        }
    ...
    protected function _prepareAttributes($rowData, $rowScope, $attributes, $rowSku, $rowStore)
    {
        ...
        foreach ($rowData as $attrCode => $attrValue) {
            ...
            if (self::SCOPE_STORE == $rowScope || self::SCOPE_NULL == $rowScope) {
        ...
        foreach ($mediaGalleryData as $productSku => $mediaGalleryRows) {
            ...
            foreach ($mediaGalleryRows as $insertValue) {
                ...
                $valueArr = array(
                    ...
                    'position' => (int)$insertValue['position'],
                    'disabled' => isset($insertValue['disabled']) ? (int)$insertValue['disabled'] : 0
                );
    ...
    {
        if ($this->_mediaGalleryAttributeId === null) {
            $adapter = $this->getConnection();
            $resource = $this->getResourceModel('eav/entity_attribute');
            ...
            $this->_mediaGalleryAttributeId = $adapter->fetchOne($select);
        }
    ...
    /**
     * Prepare data of dependencies, which will used for validation
     *
     * @param Mage_ImportExport_Model_Import_Adapter_Abstract $source
     */
    protected function _dependenciesPreparation(Mage_ImportExport_Model_Import_Adapter_Abstract $source)
    {
        $skuList = array();
