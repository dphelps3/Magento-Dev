    ...
    extends Mage_ImportExport_Model_Import_Entity_Product_Type_Abstract
{
    ...
    /**
     * Array of SKU to array of super attribute values for bunch of products
     *
     * @return Mage_ImportExport_Model_Import_Entity_Product_Type_Configurable
     */
    protected function _loadSkuSuperAttributeValuesForBunch(&$bunch)
    {
        $skuList = array();
        $skuColumn = Mage_ImportExport_Model_Import_Entity_Product::COL_SKU;

        foreach ($bunch as $row) {
            if (isset($row[$skuColumn])) {
                $skuList[] = $row[$skuColumn];
            }
        ...
        if ($this->_superAttributes && count($skuList)) {
            $attrSetIdToName   = $this->_entityModel->getAttrSetIdToName();
            $allowProductTypes = array();

            $types = Mage::getConfig()
                ->getNode('global/catalog/product/type/configurable/allow_product_types')->children();
            ...
            $collection = Mage::getResourceModel('catalog/product_collection')
                ->addFieldToFilter('sku', array('in' => $skuList))
            ...
            foreach ($collection as $product) {
                ...
                if (!isset($this->_skuSuperAttributeValues[$attrSetName][$product->getId()])) {
    ...
    public function saveData()
    {
        ...
        while ($bunch = $this->_entityModel->getNextBunch()) {
            ...
            $this->_loadSkuSuperAttributeValuesForBunch($bunch);
