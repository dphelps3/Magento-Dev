...
abstract class Mage_ImportExport_Model_Import_Adapter_Abstract implements SeekableIterator
{
    ...
    {
        if (count($this->_currentRow) < $this->_colQuantity) {
            $this->_currentRow = array_pad($this->_currentRow, $this->_colQuantity, '');
        } elseif (count($this->_currentRow) > $this->_colQuantity) {
            $this->_currentRow = array_slice($this->_currentRow, 0, $this->_colQuantity);
        }

        return array_combine($this->_colNames, $this->_currentRow);
    }
