...
class Mage_Catalog_Model_Resource_Product_Attribute_Backend_Media extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _removeDuplicates(&$result)
    {
        ...
        foreach (array_keys($result) as $index) {
            $productId = $result[$index]['product_id'];
            if (!isset($fileToId[$productId])) {
                $fileToId[$productId] = array();
            }
            if (!isset($fileToId[$productId][$result[$index]['file']])) {
                $fileToId[$productId][$result[$index]['file']] = $result[$index]['value_id'];
            } elseif ($fileToId[$productId][$result[$index]['file']] != $result[$index]['value_id']) {
