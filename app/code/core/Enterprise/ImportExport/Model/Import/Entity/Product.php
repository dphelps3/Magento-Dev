...
class Enterprise_ImportExport_Model_Import_Entity_Product extends Mage_ImportExport_Model_Import_Entity_Product
{
    ...
    /**
     * Check is url keys needed
     *
     * @param array $rowData
     * @return bool|null
     */
    protected function _isUrlKeysNeeded($rowData)
    {
        if ($this->_isUrlKeysNeeded === null) {
            if (!empty($rowData) && array_key_exists('url_key', $rowData)) {
                $this->_initUrlKeys();
                $this->_isUrlKeysNeeded = true;
            } else {
                $this->_isUrlKeysNeeded = false;
            }
        }

        return $this->_isUrlKeysNeeded;
    }
    ...
    {
        if ($this->_isUrlKeysNeeded($rowData)) {
