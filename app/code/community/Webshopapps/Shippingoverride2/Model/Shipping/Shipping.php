...
class Webshopapps_Shippingoverride2_Model_Shipping_Shipping extends Mage_Shipping_Model_Shipping
{
    ...
    protected function _addWeightIncrease(&$request,$weightIncArr) {
        ...
        if (is_array($weightIncArr) && array_key_exists('all_items',$weightIncArr)) {
    ...
    private function collectShippingRates($carrier, $request)
    {
        ...
        */
		...
		if($_SERVER["REMOTE_ADDR"] == '72.239.158.50'){
			Mage::log('collectShippingRates: ', null, 'aaron_shipping_array.log');
			Mage::log($result, null, 'aaron_shipping_array.log');
		}
        return $result;
    }
                                          ...
                                          $weightIncArr = array())
    {
        ...
        if (!empty($allItemsResults) &&
            ...
            foreach ($rates as $key => $rate) {
                ...
                if ($found && $wipeRate && !empty($rates[$key])) {
					...
					if($_SERVER["REMOTE_ADDR"] == '72.239.158.50'){
						Mage::log('shippingoverride2'. ' wiping rate '. $rate->getMethodTitle(), null, 'aaron_rate_wipe_log.log');
                        Mage::log('shippingoverride2'. ' new price: '. $rates[$key]->getNewPrice(), null, 'aaron_rate_wipe_log.log');
                        Mage::log('shippingoverride2'. ' price: '. $rates[$key]->getPrice(), null, 'aaron_rate_wipe_log.log');
					}
				...
				// Aaron Karol - Shipping Coupon Start
					$coupon_code = Mage::getSingleton('checkout/session')->getQuote()->getCouponCode();
					$conn = Mage::getSingleton('core/resource')->getConnection('core_read');
					$result = $conn->fetchCol("SELECT name FROM csmagcabinet_coupons WHERE name = \"".$coupon_code."\" LIMIT 1;");
					 //Mage::log(('Coupon detected at checkout', null, 'aaron_cab_coupon.log');
					 //Mage::log(($coupon_code, null, 'aaron_cab_coupon.log');
					 //Mage::log(($result, null, 'aaron_cab_coupon.log');
					if(isset($coupon_code) && count($result) > 0 && $coupon_code == $result[0]) {
						 //Mage::log(('Info for rate: '.$rates[$key]->getMethod() , null, 'aaron_cab_rates.log');
						 //Mage::log(('Price Before: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');

						// commercial with liftgate & appointment
						if($rates[$key]->getMethod() == 'Residential') {
							$rates[$key]->setPrice($rates[$key]->getPrice() - 300);
							 //Mage::log(('Price - 300: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
							if($rates[$key]->getPrice() < 0) {
								$rates[$key]->setPrice(0);
								 //Mage::log(('Rate < 0, setting price to 0: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
							}
								//$rates[$key]->setPrice($rates[$key]->getPrice() + 99);
								 //Mage::log(('Adding residential charge: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
						}
						// commercial with liftgate & appointment
						if($rates[$key]->getMethod() == 'Commercial_w/_Liftgate_and_Appointment') {
							$rates[$key]->setPrice($rates[$key]->getPrice() - 300);
							 //Mage::log(('Price - 300: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
							if($rates[$key]->getPrice() < 0) {
								$rates[$key]->setPrice(0);
								...
								//$rates[$key]->setPrice($rates[$key]->getPrice() + 69);
								 //Mage::log(('Adding liftgate and appointment charge: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
						}
						// commercial with liftgate
						if($rates[$key]->getMethod() == 'Commercial_w/_Liftgate_Assistance') {
							$rates[$key]->setPrice($rates[$key]->getPrice() - 300);
							 //Mage::log(('Price - 300: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
							if($rates[$key]->getPrice() < 0) {
								$rates[$key]->setPrice(0);
								...
								//$rates[$key]->setPrice($rates[$key]->getPrice() + 49);
								 //Mage::log(('Adding liftgate: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
						}
						// commercial with appointment
						if($rates[$key]->getMethod() == 'Commercial_w/_Appointment_Delivery') {
							$rates[$key]->setPrice($rates[$key]->getPrice() - 300);
							 //Mage::log(('Price - 300: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
							if($rates[$key]->getPrice() < 0) {
								$rates[$key]->setPrice(0);
								...
								//$rates[$key]->setPrice($rates[$key]->getPrice() + 20);
								 //Mage::log(('Adding appointment charge: '.$rates[$key]->getPrice(), null, 'aaron_cab_rates.log');
						}
					}
				// Aaron Karol - Shipping Coupon End
            }
		...
		if($_SERVER["REMOTE_ADDR"] == '72.239.158.50'){
			Mage::log('$specialResultsArry', null, 'aaron_shipping_array.log');
			Mage::log($specialResultsArry, null, 'aaron_shipping_array.log');
		}
