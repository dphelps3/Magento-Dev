...
class Webshopapps_Productmatrix_Model_Mysql4_Carrier_Productmatrix extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getNewRate(Mage_Shipping_Model_Rate_Request $request, &$errorText)
    {
        ...
        if (empty($zipcodeMaxLength)) {
            ...
            if ('BR' == $request->getDestCountryId()) {
                $splitPostcode = explode('-', $request->getDestPostcode());
                $postcode = $splitPostcode[0] . $splitPostcode[1];
            } elseif ('GB' == $request->getDestCountryId()) {
                $postcode = $request->getDestPostcode();//Dont shorten UK postal codes. Will break logic. PROD-87
            } else {
    ...
    protected function manipulatePricing(&$absoluteResults, $conditionName)
    {
        ...
        if ($conditionName == "highest" && !$this->_ignoreAdditionalItemPrice) {
            foreach ($absoluteResults as $key => $data) {
                ...
                if (strtolower($tempAlgorithm[0]) == ('ai') || strtolower($tempAlgorithm[0]) == ('im')) {
                    if ($data['qty'] = 1 && $data['multiprice'] > 0 && !$data['override']) {
                                      ...
                                      &$zipString, &$shortMatchPostcode, &$longMatchPostcode, &$filterCanada)
    {
        ...
        $customerPostcode = preg_replace('/\s+/', '', $customerPostcode); //PROD-96
            ...
            foreach ($indResults as $indKey => $data) {
                if ($result['delivery_type'] == $data['delivery_type']) {
                    if (!$baseResults[$key]['override'] && ($data['price'] > $baseResults[$key]['price'] || $data['override'])) { // if higher get higher
                        ...
                        $baseResults[$key]['additional_price'] += $baseResults[$key]['multiprice'] * $baseResults[$key]['qty'];//PROD-92 This must be calculated before multiprice is overwritten
                        $baseResults[$key]['multiprice'] = $data['multiprice'];
                        $baseResults[$key]['qty'] = $data['qty'];
                        $baseResults[$key]['notes'] = $data['notes'];
                        //$baseResults[$key]['priority'] = $data['priority']; //PROD-91
                        if (preg_match('/c=/i', $result['algorithm'])) { //PROD-93
                            $baseResults[$key]['method_name'] = $data['method_name'];
                        }
                    } else {
                        $baseResults[$key]['additional_price'] += $data['multiprice'] * $data['qty'];
                        /*if ($baseResults[$key]['priority'] > $data['priority']) {
                            $baseResults[$key]['priority'] = $data['priority']; //PROD-91
                        }*/
                    }
        ...
        foreach ($baseResults as $key => $result) {
            ...
            foreach ($indResults as $indKey => $data) {
                if ($result['delivery_type'] == $data['delivery_type']) {
                    ...
                    if (preg_match('/c=/i', $result['algorithm'])) {
                        if (preg_match('/alt/i', $result['algorithm'])) {
                            $baseResults[$key]['method_name'] = $data['method_name'];
                        }
                    }
                    ...
                    if ($baseResults[$key]['override']) {
                        ...
                        $baseResults[$key]['priority'] = $data['priority']; //PROD-84
                        $baseResults[$key]['package_id'] = $baseResults[$key]['package_id'] . "," . $data['package_id'];
                        $found = true;
                        break;
                    }
                                                 ...
                                                 $zipBreakdown, &$errorText, $j = 0)
    {
        ...
        */
        ...
        if (!empty($row)) {
            ...
            foreach ($row as $data) {
                if ($data['price'] == -1) {
                    $this->_addExclusion($data, $structuredItem, $errorText);
                    continue;
                }
    ...
    protected function _addExclusion($data, $structuredItem, &$errorText)
    {
        ...
        if ($skipData) {
            // day or time stops this from applying
            return;
        }
            ...
            foreach ($algorithm_array as $algorithm_single) {
                ...
                if (!empty($algorithm) && count($algorithm) == 2) {
                    ...
                    switch ($algKey) {
                        ...
                        case "i":
                            if (strtolower($algValue) == 'ignore') {
                                ...
                                if ($conditionName == 'per_package') {
                                    $data['price'] += $algValue * (ceil($structuredItem['qty']) - $data['item_from_value']);
                                } else {
                                    $data['multiprice'] = $algValue;
                                    $data['qty'] = ceil($structuredItem['qty']);
                                }
                            }
                            break;
                        case "io":
                            ...
                            if (strtolower($algValue) == 'ignore') {
                                ...
                                if (!empty($itemIncrease) && count($itemIncrease) == 2) {
                                    ...
                                    if ($conditionName == 'per_package') {
                                        ...
                                        $data['multiprice'] = $itemIncrease[1];
                                        $data['qty'] = $quotient;
                                    }
                                }
                            }
                        ...
                        case "cprice":
                            // price based
                            $priceIncrease = explode("@", $algValue);
                            if (!empty($priceIncrease) && count($priceIncrease) == 2) {
                                if ($data['price_from_value'] == -1) {
                                    $data['price_from_value'] = 0;
                                }

                                $priceDifference = $structuredItem['price'] - $data['price_from_value'];
    ...
    protected function isApplicableDay($days)
    {
        try {
            ...
            $today = date("N", Mage::app()->getLocale()->storeTimeStamp());
    ...
    private function populateStructuredItems($items, $conditionName)
    {
        ...
        foreach ($items as $item) {
            ...
            } else if ($item->getProductType() == Mage_Catalog_Model_Product_Type::TYPE_GROUPED && $this->_useGroupParent) {//PROD-86
                $parentId = Mage::getResourceSingleton('catalog/product_link')
                    ->getParentIdsByChild($item->getProduct()->getId(), Mage_Catalog_Model_Product_Link::LINK_TYPE_GROUPED);
            ...
            if ($product->getAttributeText('package_id') == 'SKU' && in_array('use_sku', $this->_options)) { //PROD-68
                ...
                $selectedOptions = Mage::helper('catalog/product_configuration')->getCustomOptions($item);

                $options = array();

                foreach ($product->getOptions() as $opt) {
                    foreach ($opt->getValues() as $value) {
                        $name = $value['default_title'];
                        $options[$name] = $value['sku'];
                    }
                }

                foreach ($selectedOptions as $option) {
                    if(array_key_exists($option['value'],$options)) {
                        $packageIdArray[] = $options[$option['value']]; //Add each option SKU to package ID array.
                    }
                }
            ...
            foreach ($websites as $websiteId) {
                ...
                if (!empty($csv)) {
                    ...
                    foreach ($csvLines as $csvLine) {
                        ...
                        } else if ((count($csvLine) == 13 || (count($csvLine) == 14 && $this->_uDrop)) && in_array('zone_lookup', $this->_options)) {
                            /**
                             * PROD-85
                             * else is ups zones
                             **/
                                    ...
                                    if ($usingZones && $this->_uDrop) {
                                        ...
                                        foreach ($splitZones as $zone) {
                                            ...
                                            $data[] = array('website_id' => $websiteId, 'dest_country_id' => $zone,
                                                ...
                                                'item_from_value' => $item_from, 'item_to_value' => $item_to,
                                                'customer_group' => $customer_group,
                                                'price' => $csvLine[$zoneOffset + 9], 'algorithm' => $csvLine[$zoneOffset + 10],
                                                'delivery_type' => $csvLine[$zoneOffset + 11], 'notes' => $csvLine[$zoneOffset + 12]);
                                        }
