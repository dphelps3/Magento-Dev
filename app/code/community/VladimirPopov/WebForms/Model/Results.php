	...
	extends Mage_Core_Model_Abstract
{
	...
	public function toHtml($recipient = 'admin', $options = array ())
	{
		...
		foreach ($fields_to_fieldsets as $fieldset)
		{
			...
			/*************************************************/
			/** Simple Solutions - Add Cart Data to Emails. **/
			/*************************************************/
			//~ $quote = Mage::getSingleton('checkout/session')->getQuote();
			//~ $cartItems = $quote->getAllVisibleItems();
			//~ if($cartItems) {
				//~ $cart = '<h2>Cart</h2><br /><table cellpadding="5" cellspacing="5"><tr><td style="font-weight:bold;">Item Sku</td><td  style="font-weight:bold;">Item Name</td><td  style="font-weight:bold;">Quantity</td></tr>';
				//~ foreach ($cartItems as $item) {
					//~ $cart .= '<tr>';
					//~ $cart .= '<td>'.$item->getSku().'</td>';
					//~ $cart .= '<td>'.$item->getName().'</td>';
					//~ $cart .= '<td>'.$item->getQty().'</td>';
					//~ $cart .= '</tr>';
		...
		/****************************************************************
		/** Simple Solutions - Check to see if Cart Data has been set. 
		/****************************************************************/
		if(isset($cart)) {
			return $head_html . $html . $cart;
		} else {
