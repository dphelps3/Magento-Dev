...
class Ebizmarts_SagePaymentsCE_Model_SageMethod extends Mage_Payment_Model_Method_Cc
{
	...
	const POST_CODE_RESULT_AVS = '650102';
	const POST_CODE_RESULT_CVV = '000063';
        const POST_CODE_RESULT_CV1 = '650104';
   ...
   private function _getSagePayInfo($orderId) {
	...
	public function capture(Varien_Object $payment, $amount)
	{
		...
		if ($result->getResponseStatus() == self::RESPONSE_CODE_APPROVED) {
		    $payment
		    	...
		    	->setLastTransId($result->getPostCodeResult())
		    ...
		    if ($result->getResponseStatusDetail()) {
		        $error = '';
		if ($result->getResponseStatus() == self::RESPONSE_CODE_REJECTED && $result->getPostCodeResult() == self::POST_CODE_RESULT_AVS) {
			$error = Mage::helper('sagepaymentsce')->__('Your billing address or name  does not match your cards billing address or the name on the card, the card was not authenticated: ');
		} else if ($result->getResponseStatus() == self::RESPONSE_CODE_NOTAUTHED && $result->getPostCodeResult() == self::POST_CODE_RESULT_CVV) {
			$error = Mage::helper('sagepaymentsce')->__('The 3 or 4 digit security code did not match the card, the card was not authenticated: ');
                } else if ($result->getResponseStatus() == self::RESPONSE_CODE_REJECTED && $result->getPostCodeResult() == self::POST_CODE_RESULT_CV1) {
                        $error = Mage::helper('sagepaymentsce')->__('The 3 or 4 digit security code did not match the card, or the cards name and billing address are incorrect, the card was not authenticated: ');
		} else if ($result->getResponseStatus() == self::RESPONSE_CODE_NOTAUTHED) {
	...
	protected function _postRequest($data, $operation)
	{
		...
		$auxdata['C_EXP'] = substr_replace($auxdata['C_EXP'],"XXXXXXXXXXXXX",0);
		$auxdata['C_CVV'] = substr_replace($auxdata['C_CVV'],"XXXXXXXXXXXXX",0);
