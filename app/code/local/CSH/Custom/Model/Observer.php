<?php
class CSH_Custom_Model_Observer{
	/**
	...
	public function saveQuoteAfter($evt){
		...
		if($quote->getCshResidential()){
			$var = $quote->getCshResidential();

			if(!empty($var)){
				$model = Mage::getModel('custom/custom_quote');
				$model->deteleByQuote($quote->getId(),'csh_residential');
				$model->setQuoteId($quote->getId());
				$model->setKey('csh_residential');
				$model->setValue($var);
				$model->save();
			}
		}
		if($quote->getCshLiftgate()){
			$var = $quote->getCshLiftgate();

			if(!empty($var)){
				$model = Mage::getModel('custom/custom_quote');
				$model->deteleByQuote($quote->getId(),'csh_liftgate');
				$model->setQuoteId($quote->getId());
				$model->setKey('csh_liftgate');
				$model->setValue($var);
				$model->save();
			}
		}
		if($quote->getCshAppointment()){
			$var = $quote->getCshAppointment();

			if(!empty($var)){
				$model = Mage::getModel('custom/custom_quote');
				$model->deteleByQuote($quote->getId(),'csh_appointment');
				$model->setQuoteId($quote->getId());
				$model->setKey('csh_appointment');
				$model->setValue($var);
				$model->save();
			}
		}
		if($quote->getCshComments2()){
			$var = $quote->getCshComments2();
            
			if(!empty($var)){
				$model = Mage::getModel('custom/custom_quote');
				$model->deteleByQuote($quote->getId(),'csh_comments2');
				$model->setQuoteId($quote->getId());
				$model->setKey('csh_comments2');
				$model->setValue($var);
				$model->save();
			}
		}
	...
	public function saveOrderAfter($evt){
		...
		if($quote->getCshResidential()){
			...
			if(!empty($var)){
				$model = Mage::getModel('custom/custom_order');
				$model->deleteByOrder($quote->getId(),'csh_residential');
				$model->setOrderId($order->getId());
				$model->setKey('csh_residential');
				$model->setValue($var);
				$model->save();
		...
		if($quote->getCshLiftgate()){
			...
			if(!empty($var)){
				$model = Mage::getModel('custom/custom_order');
				$model->deleteByOrder($quote->getId(),'csh_liftgate');
				$model->setOrderId($order->getId());
				$model->setKey('csh_liftgate');
				$model->setValue($var);
				$model->save();
		...
		if($quote->getCshAppointment()){
			...
			if(!empty($var)){
				$model = Mage::getModel('custom/custom_order');
				$model->deleteByOrder($quote->getId(),'csh_appointment');
				$model->setOrderId($order->getId());
				$model->setKey('csh_appointment');
				$model->setValue($var);
				$model->save();
		...
		if($quote->getCshComments2()){
			...
			if(!empty($var)){
				$model = Mage::getModel('custom/custom_order');
				$model->deleteByOrder($quote->getId(),'csh_comments2');
				$model->setOrderId($order->getId());
				$model->setKey('csh_comments2');
				$model->setValue($var);
				$model->save();
