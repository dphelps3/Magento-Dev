...
class Amasty_Shopby_Block_Top extends Mage_Core_Block_Template
{
    ...
    protected function _isPageHandled()
    {
		...
		//Mage::Log('origtitle: ' . $title);
		//Mage::Log('methods: ' . print_r(get_class_methods($page), true));
		
		/*
		Modified to fix category titles missing
        if ($page->getUseCat()){
		...
		*/
		// New code to fix missing category titles
		$title = $title . $titleSeparator . $page->getMetaTitle();
		$descr = $descr . $descrSeparator . $page->getMetaDescr();
		$kw = $page->getMetaKw() . $kwSeparator . $kw;
        // End new code to fix missing category titles
		
		
		//Mage::Log('newtitle: ' . $title);
