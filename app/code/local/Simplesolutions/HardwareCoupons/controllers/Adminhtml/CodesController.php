...
class Simplesolutions_HardwareCoupons_Adminhtml_CodesController extends Mage_Adminhtml_Controller_Action
{
		protected function _initAction()
		{
				$this->loadLayout()->_setActiveMenu("hardwarecoupons/codes")->_addBreadcrumb(Mage::helper("adminhtml")->__("Codes  Manager"),Mage::helper("adminhtml")->__("Codes Manager"));
				return $this;
		}
		public function indexAction()
		{
			    $this->_title($this->__("HardwareCoupons"));
		...
		public function editAction()
		{
				...
				$model = Mage::getModel("hardwarecoupons/codes")->load($id);
				if ($model->getId()) {
					Mage::register("codes_data", $model);
					$this->loadLayout();
					$this->_setActiveMenu("hardwarecoupons/codes");
					...
					$this->_addContent($this->getLayout()->createBlock("hardwarecoupons/adminhtml_codes_edit"))->_addLeft($this->getLayout()->createBlock("hardwarecoupons/adminhtml_codes_edit_tabs"));
					$this->renderLayout();
				}
				else {
					Mage::getSingleton("adminhtml/session")->addError(Mage::helper("hardwarecoupons")->__("Item does not exist."));
					$this->_redirect("*/*/");
				}
		}
		...
		public function saveAction()
		{
			...
			$post_data['hardware_ids'] = implode(',',$post_data['cat_id']);
				...
				if ($post_data) {
					...
					try {
						...
						$model = Mage::getModel("hardwarecoupons/codes")
						->addData($post_data)
						...
						$data = array(
							...
							'store_labels' => array('Free Hardware Shipping (Up to $300)')
						);
		...
		public function deleteAction()
		{
				if( $this->getRequest()->getParam("id") > 0 ) {
					try {
						$cmodel = Mage::getModel("hardwarecoupons/codes")->load($this->getRequest()->getParam("id"));
