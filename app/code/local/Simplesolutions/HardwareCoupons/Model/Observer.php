<?php
class Simplesolutions_HardwareCoupons_Model_Observer
{
	public function CheckCoupon(Varien_Event_Observer $observer)
	{
		...
		// Check to see if this is a hardware coupon
		//database read adapter
		$conn = Mage::getSingleton('core/resource')->getConnection('core_read');
		$query_results = $conn->fetchAll("SELECT * FROM csmaghardware_coupons WHERE name = \"".$couponCode."\";");
		if($query_results) {
			$eligible_hardwares = explode(',',$query_results[0]['hardware_ids']);
			//if($_SERVER['REMOTE_ADDR'] == '72.239.158.50') {
			...
			foreach ($cart->getAllItems() as $item) {
				...
				//Mage::log($query_results[0]['hardware_ids'], null, 'aaron_cab_observer.log');
			//	if($_SERVER['REMOTE_ADDR'] == '72.239.158.50') {
					Mage::log($path, null, 'aaa_shipping_coupons.log');
					Mage::log('Cab Ids Needed', null, 'aaa_shipping_coupons.log');
					Mage::log($query_results[0]['hardware_ids'], null, 'aaa_shipping_coupons.log');
			//	}
				foreach($cats as $cat) {
					//Mage::log($results, null, 'aaron_cab_observer.log');
					if(in_array($cat, $eligible_hardwares) || $cat == $query_results[0]['hardware_ids']) {
					//	if($_SERVER['REMOTE_ADDR'] == '72.239.158.50') {
