    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
	...
	public function getSimpleFreightCost($freightPriceTotal) {
		if($freightPriceTotal <= 500) {
			...
			//Mage::log("Freight Shipping Portion : ".$freightPriceTotal." * .10 = ".$shippingPriceFreight);
		} else {
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
		...
		// if we are shipping any standard items from the same warehouse as freight items,
		// we will consider the standard items to be freight
		foreach ($standardItems as $key => $product) {
			if (isset($warehouses[$product['warehouse']])) { // $warehouses only holds freight items
				$warehouses[$product['warehouse']][] = $product;
		...
		//Mage::log("Shipping Debug: ".print_r($log, true));
        return $result;
    }
