<?php
class Simplesolutions_Cabinetproduct_Model_Product_Type_Cabinet extends Mage_Catalog_Model_Product_Type_Abstract
{
	...
	public function getCabinetProducts($product = null) {		
		if (!$this->getProduct($product)->hasData($this->_keyAssociatedCategories)) {
			...
			//}
			//Mage::Log('test2');
			$associated_categories = $this->getCabinetCategories($categories);
	...
	public function getCabinetCategories($categories) {
		$output = array();
		foreach($categories as $category) {
			//$cat = new Mage_Catalog_Model_Category();
			//$cat->load($category);
			...
			if($category->hasChildren()) {
				...
				$children = $this->getCabinetCategories($this->getChildrenCollection($category->getId()));
			}
 	...
 	protected function _prepareProduct(Varien_Object $buyRequest, $product, $processMode) 
	{
        ...
        if (!$isStrictProcessMode || (!empty($productsInfo) && is_array($productsInfo))) {
                    ...
                    if(isset($productsInfo[$subProductId])) {
                        ...
                        if (!empty($qty) && is_numeric($qty)) {
                            ...
                            if ($isStrictProcessMode) {
                                ...
                                $_result[0]->addCustomOption('product_type', self::TYPE_CABINET, $product);
                                $_result[0]->addCustomOption('info_buyRequest',
                                    serialize(array(
                                        'super_product_config' => array(
                                            'product_type'  => self::TYPE_CABINET,
            ...
            if (!$isStrictProcessMode || count($associatedProductsInfo)) {
                $product->addCustomOption('product_type', self::TYPE_CABINET, $product);
