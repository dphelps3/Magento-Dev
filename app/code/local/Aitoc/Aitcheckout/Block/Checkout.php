...
class Aitoc_Aitcheckout_Block_Checkout extends Mage_Checkout_Block_Onepage_Abstract
{
    ...
    protected function _setCompactTemplate()
    {
        ...
        if ($headBlock) {
            ...
            $headBlock->addItem('skin_css', 'css/aitoc/aitcheckout/giftmessage_popup.css');
    ...
    public function setContext($context)
    {
        ...
        if (!$this->helper('aitcheckout')->isShowCheckoutInCart() || $this->helper('aitcheckout')->isDisabled()) {
            ...
            if (!$this->helper('aitcheckout')->isShowProceedToCheckoutButton()) {
                $cartBlock->getChild('top_methods')->unsetChild('checkout.cart.methods.onepage.top');
                if (!Mage::helper('aitcheckout')->isCompactDesign()) {
                    $cartBlock->getChild('methods')->unsetChild('checkout.cart.methods.onepage.bottom');
                }
            }
    ...
    public function chooseTemplate()
    {
        ...
        if ($this->getQuote()->getItemsCount()) {
            ...
            if (!Mage::getSingleton('customer/session')->isLoggedIn()) {
                ...
                if ($headBlock) {
                    $headBlock->addItem('skin_css', 'css/aitoc/aitcheckout/popup.css');
                    $headBlock->addItem('skin_js', 'js/aitoc/aitcheckout/popup/customer_popup.js');
                }
            }
        } else {
            $this->setTemplate($this->getEmptyTemplate());
    ...
    {
        return $this->_isCheckoutLoginPersistent() && Mage::helper('persistent')->isEnabled()
        && Mage::helper(
            'persistent'
