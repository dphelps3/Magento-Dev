...
class Aitoc_Aitcheckout_Model_Order_Observer
{
    ...
    public function onOnepageSaveOrderAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($request->getPost('newsletter') && $quote->getBillingAddress()->getEmail()) {
            ...
            if ($ownerId !== null && $ownerId != $customerSession->getId()) {
                $session->addError(
                    Mage::helper('aitcheckout')->__(
                        'There was a problem with the newsletter subscription: This email address is already assigned to another user.'
                    )
                );
            } else {
                ...
                if ($status == Mage_Newsletter_Model_Subscriber::STATUS_NOT_ACTIVE) {
                    $session->addSuccess(
                        Mage::helper('aitcheckout')->__(
                            'Confirmation request for newsletter subscription has been sent.'
                        )
                    );
                } else {
                    $session->addSuccess(
                        Mage::helper('aitcheckout')->__('Thank you for your newsletter subscription.')
                    );
                }
            }
        }
    ...
    function onSalesOrderPlaceBefore(Varien_Event_Observer $observer)
    {
        ...
        if (($checkoutMethod == Mage_Checkout_Model_Type_Onepage::METHOD_REGISTER)
            ...
            // Magento does not create a customer account during checkout via PayPal prior to v 1.11.1
            // thus in that case we should disable the following check to prevent unnecessary error
            if (!(version_compare(Mage::getVersion(), '1.11.1', 'lt')
                && $quote->getPayment()->getMethod() == Mage_Paypal_Model_Config::METHOD_WPP_EXPRESS
                && !$customer->getEmail())
            ) // For case when someone have fixed customer account creation in Magento prior to v.1.11.1 
            {
    ...
    public function renderReviewBlockAfterAitSaveOrder(Varien_Event_Observer $observer)
    {
        if (Mage::helper('aitcheckout')->isPaypalAdvancedAvailable()
            && !Mage::registry(
                'aitcheckout_paypal_review_block'
            )
        ) {
    ...
    protected function _getPaypalReviewBlockHtml()
    {
        ...
        if (is_object($block)) {//if module is enabled, but Checkout is disabled        
            $html = str_replace(
                Aitoc_Aitcheckout_Block_Rewrite_PaypalIframe::IFRAME_URL_REPLACEMENT,
                $block->getFrameActionUrl(),
                $html
            );
        }
