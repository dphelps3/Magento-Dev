...
class Aitoc_Aitcheckout_Helper_Captcha extends Aitoc_Aitcheckout_Helper_Abstract
{
    ...
    {
        return version_compare(Mage::getVersion(), '1.12.0.0', '>=');
    }

    public function checkIfCaptchaEnabled()
    {
        if ($this->_enabled === null) {
            $this->_enabled = $this->checkIfFormAdditionalInfoAllowed()
                && Aitoc_Aitsys_Abstract_Service::get()
                    ->isModuleActive('Mage_Captcha');
        }
    ...
    public function generateNewCaptcha($formId = false)
    {
        ...
        Mage::getSingleton('core/layout')->createBlock($captchaModel->getBlockName())->setFormId($formId)->setIsAjax(
            true
        )->toHtml();
