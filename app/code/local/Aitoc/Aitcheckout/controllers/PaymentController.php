...
$ebizmartsControllerFile = Mage::getBaseDir() . DS . 'app' . DS . 'code' . DS . 'local' . DS . 'Ebizmarts' . DS
    . 'SagePaySuite' . DS . 'controllers' . DS . 'PaymentController.php';
...
if (Mage::helper('aitcheckout/sagepay')->checkIfEbizmartsSagePaySuiteActive() && is_file($ebizmartsControllerFile)) {
    ...
    class Aitoc_Aitcheckout_PaymentController extends Ebizmarts_SagePaySuite_PaymentController
    {
        public function onepageSaveOrderAction()
        {
            ...
            if ((false === strstr(parse_url($this->_getRefererUrl(), PHP_URL_PATH), 'onestepcheckout'))
                && is_null(
                ...
                if (array_key_exists('billing', $requestParams)) {
                    $billingAddressId  = array_key_exists('billing_address_id', $requestParams)
                        ? $requestParams['billing_address_id'] : false;
                    $shippingAddressId = array_key_exists('shipping_address_id', $requestParams)
                        ? $requestParams['shipping_address_id'] : false;
                    $sameAsBilling     = array_key_exists('use_for_shipping', $requestParams['billing'])
                        ? $requestParams['billing']['use_for_shipping'] : false;
                    ...
                    if (!Mage::helper('customer')->isLoggedIn()) {
                        ...
                        if ($registration_mode == 'auto_generate_account') {
                            ...
                            $password                          = Mage::helper('onestepcheckout/checkout')
                                ->generatePassword();
                            ...
                            $this->_getQuote()->setData(
                                'password_hash',
                                Mage::getModel('customer/customer')->encryptPassword($password)
                            );
                        ...
                        if ($registration_mode == 'require_registration' || $registration_mode == 'allow_guest') {
                            if (!empty($billing_data['customer_password']) && !empty($billing_data['confirm_password'])
                                && ($billing_data['customer_password'] == $billing_data['confirm_password'])
                            ) {
                    ...
                    if (array_key_exists('subscribe_newsletter', $requestParams)
                        ...
                        $this->getSageSuiteSession()->setOscNewsletterEmail(
