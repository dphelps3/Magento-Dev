...
class Aitoc_Aitcheckout_CheckoutController extends Aitoc_Aitcheckout_Controller_Action
{
    ...
    protected function _isCaptchaCorrect()
    {
        ...
        if ($response->getBody()) {
            ...
            if ($result['error'] == 1) {
                ...
                $response->setBody(
                    Mage::helper('core')->jsonEncode($this->_extractStepOutput('billing', $result->__toArray()))
                );
        ...
        if ($this->getRequest()->isPost()) {
            ...
            switch ($currentStep) {
                ...
                case 'deliverydate':
                     ...
                     */
                    $result = Mage::getModel('adjdeliverydate/step')->process('shippingMethod');
                    if (isset($result['error'])) {
                        $validDate                     = $this->_getValidDate();
                        $aditionalResult['valid_date'] = $this->_getValidDate();
                        $this->getRequest()->setPost('adj', array('delivery_date' => $aditionalResult['valid_date']));
                    }
                ...
                case 'reward':
                    $inputArray        = $this->getRequest()->getPost();
                    $inputPaymentArray = isset($inputArray['payment']) ? $inputArray['payment'] : array();
                    Mage::dispatchEvent(
                        'sales_quote_payment_import_data_before',
                        array(
                            'input'   => new Varien_Object($inputPaymentArray),
                            'payment' => new Varien_Object(array('quote' => $this->_getOnepage()->getQuote()))
                        )
                    );
                    break;
            }
            ...
            try {
                $saveFunction = isset($this->_sectionSaveFunctions[$currentStep])
                    ? $this->_sectionSaveFunctions[$currentStep] : null;
                $resolve      = false;
                $result       = $saveFunction ? $this->_getOnepage()->$saveFunction($data, $customerAddressId)
                    : array();
            ...
            if (!$result && $currentStep == 'shipping_method') {
                Mage::dispatchEvent(
                    'checkout_controller_onepage_save_shipping_method',
                    array('request' => $this->getRequest(), 'quote' => $this->_getOnepage()->getQuote())
                );
            }
    ...
    {
        if (Mage::helper('aitcheckout')->isModuleEnabled('AdjustWare_Deliverydate')
            && Mage::getStoreConfigFlag(
                'checkout/adjdeliverydate/enabled'
            )
        ) {
