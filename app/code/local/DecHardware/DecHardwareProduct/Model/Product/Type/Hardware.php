<?php
class DecHardware_DecHardwareProduct_Model_Product_Type_Hardware extends Mage_Catalog_Model_Product_Type_Abstract
{
	...
	public function getHardwareProducts($product = null) {
		if (!$this->getProduct($product)->hasData($this->_keyAssociatedCategories)) {
			...
			//}
			//Mage::Log('categories' . print_r($categories, true));
			//Mage::Log('test1');
			//Mage::Log('associated_category' . $associated_category);

			//Mage::Log(count($categories));
			//foreach($categories as $category) {
				//$cat = new Mage_Catalog_Model_Category();
				//$cat->load($category->getId());
			//	Mage::Log('categoryId ' . $category->getId());
			//}
			//Mage::Log('test2');
			$associated_categories = $this->getHardwareCategories($categories);
	...
	public function getHardwareCategories($categories) {
		$output = array();
		foreach($categories as $category) {
			//$cat = new Mage_Catalog_Model_Category();
			//$cat->load($category);
			...
			if($category->hasChildren()) {
				...
				//$_category = $category_model->load($category);
				//$children = $category_model->getResource()->getChildren($_category);
				$children = $this->getHardwareCategories($this->getChildrenCollection($category->getId()));
			}
     ...
     * Perform standard preparation process and add logic specific to  product type.
     *
     * @param Varien_Object $buyRequest
     * @param Mage_Catalog_Model_Product $product
 	...
 	protected function _prepareProduct(Varien_Object $buyRequest, $product, $processMode)
	{
        ...
        if (!$isStrictProcessMode || (!empty($productsInfo) && is_array($productsInfo))) {
                    ...
                    if(isset($productsInfo[$subProductId])) {
                        ...
                        if (!empty($qty) && is_numeric($qty)) {
                            ...
                            if ($isStrictProcessMode) {
                                ...
                                $_result[0]->addCustomOption('product_type', self::TYPE_HARDWARE, $product);
                                $_result[0]->addCustomOption('info_buyRequest',
                                    serialize(array(
                                        'super_product_config' => array(
                                            'product_type'  => self::TYPE_HARDWARE,
            ...
            if (!$isStrictProcessMode || count($associatedProductsInfo)) {
                $product->addCustomOption('product_type', self::TYPE_HARDWARE, $product);
