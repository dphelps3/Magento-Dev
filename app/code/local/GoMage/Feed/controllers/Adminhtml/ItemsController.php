...
class GoMage_Feed_Adminhtml_ItemsController extends Mage_Adminhtml_Controller_Action {
	...
	public function ajaxuploadAction() {
		...
		try {
			...
			$path = Mage::getBaseDir() . DS . 'media' . DS . 'productsfeed' . DS . 'tmp';
			if (! file_exists($path)) {
				mkdir($path);
				chmod($path, 0755);
			}
		...
		if (($file = $this->getRequest()->getParam('file')) && ($id = $this->getRequest()->getParam('id'))) {
			...
			try {
				...
				if ($this->getRequest()->getParam('section') == 1)
					$data = file_get_contents(Mage::getBaseDir() . DS . 'media' . DS . 'productsfeed' . DS . 'examples' . DS . $file);
				else
					$data = file_get_contents(Mage::getBaseDir() . DS . 'media' . DS . 'productsfeed' . DS . 'tmp' . DS . $file);
