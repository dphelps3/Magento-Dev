var AitReview = Class.create(Step,
{
    initCart: function(productOptionsCssQuery, qtyCssQuery, increaseCssQuery, decreaseCssQuery, removeCssQuery, wishlistBtnQuery)
    {
        ...
        $$('.options-list>li').each(function(element){
            element.observe('click', this.optionClick.bind(element));
        }.bind(this));
    ...
    {
        var input = element.toElement.childNodes[0];
    ...
    deleteFromReviewPost: function(block, event)
    {
        this.removeStep('shipping_method');
        this.addReloadSteps(['payment', 'review']);
        
        var params = Object.toQueryString({step : this.name, reload_steps : this.reloadSteps.join(',')});
        var code = Event.element(event).up().rel;
        this.loadWaiting();
        ...
        var request = new Ajax.Request(
            this.urls.deleteGiftUrl + block + '/' + code,
            {
                method:'post',
                parameters: params,
                onComplete: this.onUpdateChild,
