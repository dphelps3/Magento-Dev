...
IWD.QuickView.Processor = {
    ...
    initShowDialog: function () {
        ...
        if (IWD.QuickView.mode == 'aac' || IWD.QuickView.mode == 'qv_aac') {
            $ji(document).on('click', '.btn-cart:not(#product_addtocart_form .btn-cart, .block-reorder .btn-cart), .iwd-btn-add-to-cart', function (e) {
                e.preventDefault();
...
//                }
                ...
                if (link.indexOf('checkout/cart/configure') != -1) {
                    e.preventDefault();
                    protocol = link.substr(link.indexOf('://') + 1);
                    makeRequest(protocol, 'aac', true);
                }
            });
    ...
    request: function (link, edit, mode) {
        ...
        $ji.get(link, {
            ...
            "edit": edit,
            "iwd_qv_mode": mode
        }, IWD.QuickView.Processor.parseResponse, 'json');
    },
    ...
    initSubmitModalForm: function () {
        $ji(document).on('submit', '#iwd_qv_product_addtocart_form_modal', function (event) {
            ...
            if (modalProductForm.validator && modalProductForm.validator.validate()) {
                protocol = $ji(this).prop('action').substr($ji(this).prop('action').indexOf('://') + 1);
                IWD.QuickView.Processor.sendRequestFormAddToCart(protocol, $ji('#iwd_qv_product_addtocart_form_modal'));
            }
        });
    },
            ...
            productAddToCartForm = {
                submit: function (button) {
                    ...
                    if (productForm.validator && productForm.validator.validate()) {
                        ...
                        protocol = $ji('#product_addtocart_form').prop('action').substr($ji('#product_addtocart_form').prop('action').indexOf('://') + 1);
                        IWD.QuickView.Processor.sendRequestFormAddToCart(protocol, $ji('#product_addtocart_form'));
                    }
                },
    ...
    sendRequestFormAddToCart: function (url, $form) {
        ...
        var to = url.indexOf('?') === -1 ? url  : url; // If you use magento EE 1.12.0.2 please comment this line
        $ji.post(
            to,
            formData,
            IWD.QuickView.Processor.parseResponse,
            'json'
        ).done(
            function(){
                console.log("Success");
        ...
        //$ji.post(url, IWD.QuickView.Processor.parseResponse, 'json');  //for magento EE 1.12.0.2
    },
    ...
    parseResponse: function (response) {
        ...
        if(currentLocation.pathname == "/wishlist/") {
            var wislistItems = $ji('#wishlist-table tbody tr');
            $ji('#item_' + response.itemId).remove();

            console.log(currentLocation.origin);
            if (wislistItems.length == 0) {
                window.location.href = currentLocation.origin + "/checkout/cart/";
            }
        }
...
//                    $ji('#iwd-qv-wrapper').html(response.confirmation);
    ...
    parseResponseShoppingCart: function (response) {
        if (typeof(response.shopping_cart) != "undefined" && response.shopping_cart !== false) {
            ...
            $ji('.cart').replaceWith(response.shopping_cart).promise().done(function () {
                $ji('input[name^="cart"]').focus(function () {
                    $ji(this).siblings('button').fadeIn();
                });
            });
    ...
    parseResponseHeader: function (response) {
        /** header - full update for correct display  shoppingcart **/
    ...
    parseResponseUpdateTopDropdown: function (response) {
        if (typeof(response.dropdown) != "undefined") {
            if (IWD.QuickView.config.useDefaultDropDown == true) {
                $ji('.header-minicart, wrrapper-ajaxcart-dropdown').html(response.dropdown).promise().done(function () {
                    if (response.show_dropdown == true) {
                        setTimeout(function () {
                            $ji('.skip-link.skip-cart, top-link-cart').trigger('click');
                        }, 200);
                    }
                });
...
IWD.QuickView.Decorator = {
    ...
    init: function () {
        ...
        if (IWD.QuickView.config.useDefaultDropDown == false) {
            this.moveDropdown();
        }
        //this.clearPrice();
        //this.removeOpen();
        ...
        if (IWD.QuickView.mode != 'aac') {
            jQueryIWD(document).on('mouseover', IWD.QuickView.config.qv_selector, function (e) {
                ...
                if (_this.children('.iwd-quick-view-button').length == 0) {
                    ...
                    protocol = link.substr(link.indexOf('://') + 1);
                    _this.append(jQueryIWD(IWD.QuickView.button_html).data('link', protocol));
                }
            });
        }
        if (IWD.QuickView.mode != 'qv') {
            $ji('.btn-cart, .iwd-btn-add-to-cart').not('#product_addtocart_form .btn-cart').each(function () {
                ...
                if (value != undefined) {
                    if (value.indexOf('setLocation') != -1) {
                        ...
                        protocol = value.substr(value.indexOf('://') + 1);
                        $ji(this).removeAttr('onclick').data('link', protocol);
                    }
                }
            });
        }
