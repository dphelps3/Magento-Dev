...
function get_tr_from_sku(sku, qty) {
	if (!sku_info[sku]) { // if the sku hasn't been stored, we need to get it
		...
		if (qty > 1) {
			return '<tr><td class="qty_td">' + qty + ' x </td><td class="sku_td"><!--<p>-->' + sku + '<!--</p><p class="unit_price">--><br />@ $' + price + '<!--</p>--></td><!--<td class="desc_td">' + sku_info[sku].nm + '</td>--><td class="price_td">$' + t_price + '<input type="hidden" name="keys" value="' + key + '" /><input type="hidden" name="qty[' + key + ']" value="' + qty + '" /></td></tr>'
		} else {
