	...
	Raphael.fn.freeTransform = function(subject, options, callback) {
		...
		var ft = subject.freeTransform = {
			...
			attrs: {
				x: bbox.x,
				y: bbox.y,
				size: { y: bbox.width, x: bbox.height },
		...
		ft.showHandles = function() {
			...
			draggables.map(function(draggable) {
				draggable.drag(function(dx, dy) {
					...
					// Here is where we put collision detection and decide if we really want to move the object.
					if ( !detectCollision(subject.node.id, ft.offset.translate.x, ft.offset.translate.y) ) {
		...
		*/
		...
		function getBBox() {
			var rad = {
				y: ( ft.attrs.rotate      ) * Math.PI / 180,
				x: ( ft.attrs.rotate + 90 ) * Math.PI / 180
			};
