...
function updateCabinetTexture(tex, sku) {
	cab_tex = tex;
	cab_line = sku;
	...
	for (var item in product.data) {
		if ( product.data[item].sku.indexOf("8.") > -1 && product.data[item].sku.indexOf(cab_line) > -1) {
			...
			if (typeof d !== 'undefined' && c.image !== 'no_selection') {
				if (d.type !== 'x') {
					html_string += '<input type="radio" style="display:none;" onclick="addCabinet(\x27' + c.sku + '\x27)" name="addcabinet' + item + '" id="addcabinet_opt' + item + '" value="' + c.sku + '"/>';
					html_string += '<label for="addcabinet_opt' + item + '">' + d.description + '<img class="lazy" src="' + media_dir + c.image + '" width="96" height="96" style="z-index: -1;"/></label>';
				}
			}
		}
	}	
	...
	for (var item in product.data) {
		if ( product.data[item].sku.indexOf("CAB.") > -1 ) {
			...
			if( c.finish && c.finish !== 'no_selection') {
				html_string += '<input type="radio" style="display:none;" onclick="updateCabinetTexture(\x27' + (media_dir + c.finish) + '\x27,\x27' + c.sku.substring(4) + '\x27)" name="cabinetline_opt' + item + '" id="cabinetline_opt' + item + '" value="' + c.sku + '"/>';
				html_string += '<label for="cabinetline_opt' + item + '">' + c.name + '<img class="lazy" src="' + media_dir + c.finish + '" width="96" height="96" style="z-index: -1;"/></label>';
			}
		}
	}	
...
function Zone (typ, sdoor, sdrawer, ssku, px, py, nheight, nwidth, ndepth, nrotation, nbase_height, do_save) {
	...
	var mesh = buildShape(type, inchesToScreen(scope.height), inchesToScreen(scope.width), inchesToScreen(scope.depth), inchesToScreen(scope.x), inchesToScreen(scope.y), inchesToScreen(scope.base_height), sku, scope.door, scope.drawer, rotation, material);

	addObject(mesh);
	
	var innershape = paper.rect(inchesToScreen(scope.x) - inchesToScreen(scope.width / 2) + (screen_width / 2), inchesToScreen(scope.y) - inchesToScreen(scope.depth / 2) + (screen_height / 2), inchesToScreen(scope.width), inchesToScreen(scope.depth));
	...
	this.build = function() {
		if (type === 'winerack' || type === 'winerackvertical') {
	...
	var move = function (dx, dy) {
		...
		var r1_x = (rotation === 90 || rotation === 270) ? Number(innershape.attr('x')) + width - depth : Number(innershape.attr('x')),
			r1_y = (rotation === 90 || rotation === 270) ? Number(innershape.attr('y')) - width + depth : Number(innershape.attr('y')),
			r1_h = (rotation === 90 || rotation === 270) ? Number(inchesToScreen(width)) : Number(inchesToScreen(depth)),
			r1_w = (rotation === 90 || rotation === 270) ? Number(inchesToScreen(depth)) : Number(inchesToScreen(width));
		...
		/* x = x < (screen_width / 2 - floor_size / 2) ? (screen_width / 2 - floor_size / 2) : x > (screen_width / 2 + floor_size / 2) - r1_w ? (screen_width / 2 + floor_size / 2) - r1_w : x; */
        var y = (parseFloat(this.oy) + parseFloat(dy));
		/* y = y < (screen_height / 2 - floor_size / 2) ? (screen_height / 2 - floor_size / 2) : y > (screen_height / 2 + floor_size / 2) - r1_h ? (screen_height / 2 + floor_size / 2) - r1_h : y; */
		...
		for (var i = 0; i < z.length; i++) {
			var in_height_box = false;
			if (z[i].base_height === source.base_height) {
				in_height_box = true;
			}
			if (z[i].base_height < source.base_height + source.height && z[i].base_height > source.base_height) {
				in_height_box = true;
			}
			if (z[i].base_height + z[i].height > source.base_height && z[i].base_height < source.base_height) {
				in_height_box = true;
			}
			
			if(z[i].id !== id && in_height_box) {
				var s = z[i].getShape();
				r2_x = (z[i].rotation === 90 || z[i].rotation === 270) ? Number(s.attr('x')) + z[i].width - z[i].depth : Number(s.attr('x'));
				r2_y = (z[i].rotation === 90 || z[i].rotation === 270) ? Number(s.attr('y')) - z[i].width + z[i].depth : Number(s.attr('y'));
				r2_w = (z[i].rotation === 90 || z[i].rotation === 270) ? Number(inchesToScreen(z[i].depth)) : Number(inchesToScreen(z[i].width));
				r2_h = (z[i].rotation === 90 || z[i].rotation === 270) ? Number(inchesToScreen(z[i].width)) : Number(inchesToScreen(z[i].depth));

				if (!x_collide) {
					x_collide = rect_collision(r2_x, r2_y, r2_w, r2_h, x, r1_y, r1_w, r1_h);
				}
				if (!y_collide) {
					y_collide = rect_collision(r2_x, r2_y, r2_w, r2_h, r1_x, y, r1_w, r1_h);
				}
		...
		if (!x_collide) {
			if ((this.stuckx && (!in_range(y, r2_y, r2_h) || Math.abs(x - r1_x) < r2_w)) || !this.stuckx ) {
				this.attr({x:x});
				this.stuckx = false;
			}								  
		} else {
		...
		if (!y_collide) {
			if ((this.stucky && (!in_range(x, r2_x, r2_w) || Math.abs(y - r1_y) < r2_h)) || !this.stucky ) {
				this.attr({y:y});
				this.stucky = false;
			} 
		} else {
		...
		var xSnap = Snap.snapTo(inchesToScreen(3), this.attr('x'), 100000000);
		var ySnap = Snap.snapTo(inchesToScreen(3), this.attr('y'), 100000000);
		...
		/* Fix positioning if rotated */
		this.transform("t0,0R" + rotation);
	...
	function print(subject, events) {
		if ( subject ) {
			$('#debug').html(events.toString());
			scope.x = Math.round(screenToInches(inchesToScreen(scope.base_x) + (subject.attrs.translate.x)));
			scope.y = Math.round(screenToInches(inchesToScreen(scope.base_y) + (subject.attrs.translate.y)));
			x = scope.x;
			y = scope.y;
								
			rotation = subject.attrs.rotate;
			scope.rotation = rotation;
			/* scene.remove(mesh);
			mesh = buildShape(type, inchesToScreen(scope.height), inchesToScreen(scope.width), inchesToScreen(scope.depth), inchesToScreen(scope.x), inchesToScreen(scope.y), inchesToScreen(scope.base_height), sku, door, rotation, material);			
			addObject(mesh); */
			
			/* mesh.rotateOnAxis(axis, degreesToRadians(rotation)); */
			mesh.rotation.y = degreesToRadians(rotation);
...
function save(itm) {
	...
	var data = {"line": cab_line, "items": cab, "floor": floor_tex};
...
function load() {
	...
	if(value){
		...
		updateCabinetTexture(getCabinetLineTexture(), cab_line);
	}
}
 ...
 Drawer rules:
	...
	*/
	...
	if ( shape === 'ovencabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,3, type, 'oven');
		Object.add(door);
	}
	if ( shape === 'drawerbasecabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 0,3, type, 'base', drawer);
		Object.add(door);
	}
	...
	if ( shape === 'diagcornerbasecabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 1,1, type, 'diag', drawer);
		Object.add(door);
	}
	if ( shape === 'diagcornerwallcabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 1,1, type, 'diagwall', drawer);
		Object.add(door);
	}
	if ( shape === 'easyreachcornerbasecabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,0, type, 'easyreach', drawer);
		Object.add(door);
	}
	if ( shape === 'leftbaseblindcornercabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,2, type, 'blindleft', drawer);
		Object.add(door);
	}	
	if ( shape === 'rightbaseblindcornercabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,2, type, 'blindright', drawer);
		Object.add(door);
	}	
	if ( shape === 'leftwallblindcornercabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,0, type, 'blindleft', drawer);
		Object.add(door);
	}	
	if ( shape === 'rightwallblindcornercabinet' ) {
		...
		/*/ Make doors/drawers*/
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,0, type, 'blindright', drawer);
		Object.add(door);
	}	
	if ( shape === 'sinkbasecabinet' ){
		...
		if (width < inchesToScreen(60)) {
			var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,2, type, 'sinkbase', drawer);
			Object.add(door);
		} else {
			var door = addDoorsAndDrawers(height, width, depth, base_height, material, 4,2, type, 'sinkbase', drawer);
			Object.add(door);
		}
	}
	if ( shape === 'cornerendbasecabinet' ){
		...
		Object.add(mesh);
		var door = addDoorsAndDrawers(height, width, depth, base_height, material, 2,0, type, 'end', drawer);
		Object.add(door);
	}
...
function cutRackHole(mesh, x, y, height, width, material) {
	...
	var cutBox;
	var Points = [];
		Points.push( new THREE.Vector2 ( inchesToScreen(2), 0 ) );
		Points.push( new THREE.Vector2 ( inchesToScreen(4), inchesToScreen(2) ) );
		Points.push( new THREE.Vector2 ( inchesToScreen(2), inchesToScreen(4) ) );
		Points.push( new THREE.Vector2 ( 0 , inchesToScreen(2) ) );
	var Shape = new THREE.Shape( Points );
...
function addRack(height, width, depth, base_height, material, type) {
	...
	if (type === 'diagonal') {
		...
		for ( i = inchesToScreen(3); i < width; i += inchesToScreen(3) ) {
			var geom = new THREE.ExtrudeGeometry( Shape, extrusionSettings );
			geom.computeFaceNormals();
			geom.center();
			mesh = new THREE.Mesh( geom , material );
			mesh.position.x = i - (width / 2);
			mesh.position.y = (height / 2) + base_height;
...
cab_def.push({sku:'8.BBC36/39.', description:'Base Cabinet 36" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:36, depth:24, base_height:0});
cab_def.push({sku:'8.BBC39.', description:'Base Cabinet 39" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:39, depth:24, base_height:0});
cab_def.push({sku:'8.BBC39/42.', description:'Base Cabinet 39" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:39, depth:24, base_height:0});
cab_def.push({sku:'8.BBC39/45.', description:'Base Cabinet 36" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:36, depth:24, base_height:0});
cab_def.push({sku:'8.BBC42.', description:'Base Cabinet 42" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:42, depth:24, base_height:0});
cab_def.push({sku:'8.BBC42/45.', description:'Base Cabinet 42" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:42, depth:24, base_height:0});
cab_def.push({sku:'8.BBC45.', description:'Base Cabinet 45" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:45, depth:24, base_height:0});
cab_def.push({sku:'8.BBC45/48.', description:'Base Cabinet 45" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:45, depth:24, base_height:0});
cab_def.push({sku:'8.BBC48/51.', description:'Base Cabinet 48" W x 34.5" H x 24" D', category:'Base Cabinet', type:'[left/right]baseblindcornercabinet', door:'default', height:34.5, width:48, depth:24, base_height:0});
...
cab_def.push({sku:'8.BEC24.', description:'Base Cabinet 24" W x 34.5" H x 24" D', category:'Base Cabinet', type:'cornerendbasecabinet', door:'default', height:34.5, width:24, depth:24, base_height:0});
cab_def.push({sku:'8.BEOS9L.', description:'Base Cabinet 9" W x 34.5" H x 24" D', category:'Base Cabinet', type:'baseendshelfflip', door:'default', height:34.5, width:9, depth:24, base_height:0});
cab_def.push({sku:'8.BEOSR.', description:'Base Cabinet 9" W x 34.5" H x 24" D', category:'Base Cabinet', type:'baseendshelf', door:'default', height:34.5, width:9, depth:24, base_height:0});
...
cab_def.push({sku:'8.BMC30.', description:'Base Cabinet 30" W x 34.5" H x 24" D', category:'Base Cabinet', type:'basemicrowave', door:'default', height:34.5, width:30, depth:24, base_height:0});
...
cab_def.push({sku:'8.DCB36.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'diagcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.DCB42.', description:'Base Cabinet 42" W x 34.5" H x 42" D', category:'Base Cabinet', type:'diagcornerbasecabinet', door:'default', height:34.5, width:42, depth:42, base_height:0});
cab_def.push({sku:'8.ECB33.', description:'Base Cabinet 33" W x 34.5" H x 33" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:33, depth:33, base_height:0});
cab_def.push({sku:'8.ECB36.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.ECB36L.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.ECB36R.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.EMB36.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.ISL6030A.', description:'Base Cabinet 60" W x 34.5" H x 30" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:60, depth:30, base_height:0});
cab_def.push({sku:'8.ISL6030B.', description:'Base Cabinet 60" W x 34.5" H x 30" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:60, depth:30, base_height:0});
cab_def.push({sku:'8.ISL6036A.', description:'Base Cabinet 60" W x 34.5" H x 36" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:60, depth:36, base_height:0});
cab_def.push({sku:'8.ISL6036B.', description:'Base Cabinet 60" W x 34.5" H x 36" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:60, depth:36, base_height:0});
cab_def.push({sku:'8.ISL7230C.', description:'Base Cabinet 72" W x 34.5" H x 30" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:72, depth:30, base_height:0});
cab_def.push({sku:'8.ISL7230D.', description:'Base Cabinet 72" W x 34.5" H x 30" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:72, depth:30, base_height:0});
cab_def.push({sku:'8.ISL7236C.', description:'Base Cabinet 72" W x 34.5" H x 36" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:72, depth:36, base_height:0});
cab_def.push({sku:'8.ISL7236D.', description:'Base Cabinet 72" W x 34.5" H x 36" D', category:'Base Cabinet', type:'x', door:'default', height:34.5, width:72, depth:36, base_height:0});
cab_def.push({sku:'8.LSB33.', description:'Base Cabinet 33" W x 34.5" H x 33" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:33, depth:33, base_height:0});
cab_def.push({sku:'8.LSB33R.', description:'Base Cabinet 33" W x 34.5" H x 33" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:33, depth:33, base_height:0});
cab_def.push({sku:'8.LSB36.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.LSB36L.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.LSB36R.', description:'Base Cabinet 36" W x 34.5" H x 36" D', category:'Base Cabinet', type:'easyreachcornerbasecabinet', door:'default', height:34.5, width:36, depth:36, base_height:0});
cab_def.push({sku:'8.MC3018.', description:'Microwave 30" W x 18" H x 12" D', category:'Microwave', type:'microwaveshelf', door:'default', height:18, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.MC303315.', description:'Microwave 30" W x 3315" H x 15" D', category:'Microwave', type:'wallmicrowave', door:'default', height:3315, width:30, depth:15, base_height:54});
cab_def.push({sku:'8.MC304515.', description:'Microwave 30" W x 45" H x 15" D', category:'Microwave', type:'wallmicrowave', door:'default', height:45, width:30, depth:15, base_height:54});
...
cab_def.push({sku:'8.WBC2430.', description:'Wall Cabinet 24" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:24, depth:12, base_height:54});
cab_def.push({sku:'8.WBC2436.', description:'Wall Cabinet 24" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:24, depth:12, base_height:54});
cab_def.push({sku:'8.WBC2442.', description:'Wall Cabinet 24" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:24, depth:12, base_height:54});
cab_def.push({sku:'8.WBC2730.', description:'Wall Cabinet 30" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WBC2736.', description:'Wall Cabinet 30" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WBC2742.', description:'Wall Cabinet 30" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3030.', description:'Wall Cabinet 30" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3036.', description:'Wall Cabinet 30" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3042.', description:'Wall Cabinet 30" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3330.', description:'Wall Cabinet 33" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:33, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3336.', description:'Wall Cabinet 33" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:33, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3342.', description:'Wall Cabinet 33" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:33, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3630.', description:'Wall Cabinet 39" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:39, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3636.', description:'Wall Cabinet 36" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:36, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3642.', description:'Wall Cabinet 36" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:36, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3930.', description:'Wall Cabinet 39" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:39, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3936.', description:'Wall Cabinet 39" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:39, depth:12, base_height:54});
cab_def.push({sku:'8.WBC3942.', description:'Wall Cabinet 39" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:39, depth:12, base_height:54});
cab_def.push({sku:'8.WBC4230.', description:'Wall Cabinet 42" W x 30" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:30, width:42, depth:12, base_height:54});
cab_def.push({sku:'8.WBC4236.', description:'Wall Cabinet 42" W x 36" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:36, width:42, depth:12, base_height:54});
cab_def.push({sku:'8.WBC4242.', description:'Wall Cabinet 42" W x 42" H x 12" D', category:'Wall Cabinet', type:'[left/right]wallblindcornercabinet', door:'default', height:42, width:42, depth:12, base_height:54});
...
cab_def.push({sku:'8.WDC2418.', description:'Wall Cabinet 24" W x 18" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:18, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2430.', description:'Wall Cabinet 24" W x 30" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:30, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2430GD.', description:'Wall Cabinet 24" W x 30" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'glass', height:30, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2430MD.', description:'Wall Cabinet 24" W x 30" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'mullion', height:30, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2430ND.', description:'Wall Cabinet 24" W x 30" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:30, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2436.', description:'Wall Cabinet 24" W x 36" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:36, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2436GD.', description:'Wall Cabinet 24" W x 36" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'glass', height:36, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2436MD.', description:'Wall Cabinet 24" W x 36" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'mullion', height:36, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2436ND.', description:'Wall Cabinet 24" W x 36" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:36, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2442.', description:'Wall Cabinet 24" W x 42" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:42, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2442GD.', description:'Wall Cabinet 24" W x 42" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'glass', height:42, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2442MD.', description:'Wall Cabinet 24" W x 42" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'mullion', height:42, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2442ND.', description:'Wall Cabinet 24" W x 42" H x 24" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:42, width:24, depth:24, base_height:54});
cab_def.push({sku:'8.WDC2730.', description:'Wall Cabinet 27" W x 30" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:30, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2730ND.', description:'Wall Cabinet 27" W x 30" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:30, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2736.', description:'Wall Cabinet 27" W x 36" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:36, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC273615.', description:'Wall Cabinet 27" W x 36" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:36, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2736GD.', description:'Wall Cabinet 27" W x 36" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'glass', height:36, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2736MD.', description:'Wall Cabinet 27" W x 36" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'mullion', height:36, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2736ND.', description:'Wall Cabinet 27" W x 36" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:36, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2742.', description:'Wall Cabinet 27" W x 42" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:42, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC274215.', description:'Wall Cabinet 27" W x 42" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:42, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2742GD.', description:'Wall Cabinet 27" W x 42" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'glass', height:42, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2742MD.', description:'Wall Cabinet 27" W x 42" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'mullion', height:42, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2742ND.', description:'Wall Cabinet 27" W x 42" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:42, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2748.', description:'Wall Cabinet 27" W x 48" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'default', height:48, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WDC2748ND.', description:'Wall Cabinet 27" W x 48" H x 27" D', category:'Wall Cabinet', type:'diagcornerwallcabinet', door:'none', height:48, width:27, depth:27, base_height:54});
cab_def.push({sku:'8.WEC1230.', description:'Wall Cabinet 12" W x 30" H x 12" D', category:'Wall Cabinet', type:'x', door:'default', height:30, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WEC1236.', description:'Wall Cabinet 12" W x 36" H x 12" D', category:'Wall Cabinet', type:'x', door:'default', height:36, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WEC1242.', description:'Wall Cabinet 12" W x 42" H x 12" D', category:'Wall Cabinet', type:'x', door:'default', height:42, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WECD1230.', description:'Wall Cabinet 12" W x 30" H x 12" D', category:'Wall Cabinet', type:'x', door:'default', height:30, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WECD1236.', description:'Wall Cabinet 12" W x 36" H x 12" D', category:'Wall Cabinet', type:'x', door:'default', height:36, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WECD1242.', description:'Wall Cabinet 12" W x 42" H x 12" D', category:'Wall Cabinet', type:'x', door:'default', height:42, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WEOS1242.', description:'Wall Cabinet 12" W x 42" H x 12" D', category:'Wall Cabinet', type:'wallshelf[/flip]', door:'default', height:42, width:12, depth:12, base_height:54});
cab_def.push({sku:'8.WEOS930.', description:'Wall Cabinet 9" W x 30" H x 12" D', category:'Wall Cabinet', type:'wallshelf[/flip]', door:'default', height:30, width:9, depth:12, base_height:54});
cab_def.push({sku:'8.WEOS936.', description:'Wall Cabinet 9" W x 36" H x 12" D', category:'Wall Cabinet', type:'wallshelf[/flip]', door:'default', height:36, width:9, depth:12, base_height:54});
...
cab_def.push({sku:'8.WMC3030.', description:'Microwave 30" W x 30" H x 12" D', category:'Microwave', type:'wallmicrowave', door:'default', height:30, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WMC3036.', description:'Microwave 30" W x 36" H x 12" D', category:'Microwave', type:'wallmicrowave', door:'default', height:36, width:30, depth:12, base_height:54});
cab_def.push({sku:'8.WMC3042.', description:'Microwave 30" W x 42" H x 12" D', category:'Microwave', type:'wallmicrowave', door:'default', height:42, width:30, depth:12, base_height:54});
